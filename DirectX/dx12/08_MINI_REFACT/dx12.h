...
struct Device {
	...
	ID3D12CommandAllocator    *allocators[MAX_BACKBUFFER] = {};
	ID3D12CommandAllocator    *allocator   = nullptr;
	ID3D12GraphicsCommandList *commandlists[MAX_BACKBUFFER] = {};
	...
	FenceData fencedata[MAX_BACKBUFFER] = {};
	...
	Device(HWND hWnd) {
		...
		//Startup
		for(int i = 0 ; i < MAX_BACKBUFFER; i++) {
			auto *data = &fencedata[i];
			device->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT, IID_PPV_ARGS(&allocators[i]));
			device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, allocators[i], nullptr, IID_PPV_ARGS(&commandlists[i]) );
			device->CreateFence(0, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&data->fence));
			data->hEvent = CreateEventEx(NULL, FALSE, FALSE, EVENT_ALL_ACCESS);
			/*
			commandlists[i]->Close();
			queue->Signal(data->fence, 0);
			data->fence->SetEventOnCompletion(0, data->hEvent);
	...
	void PrepareExecuteCommandBuffer() {
		UINT index = fenceCounter % MAX_BACKBUFFER;
