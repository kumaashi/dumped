...
struct DescriptorBufferInfo : VkDescriptorBufferInfo {
	...
	VkWriteDescriptorSet GetWriteDescriptorSet(VkDescriptorSet descSet, uint32_t binding, VkDescriptorType descType = VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER) {
		VkWriteDescriptorSet ret = {};
		ret.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET;
		ret.dstSet = descSet;
		ret.descriptorType = descType;
...
struct Buffer {
	...
	Buffer(VkDevice device, VkDeviceSize size, VkBufferUsageFlags flags = VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT) {
...
struct CommandBuffer {
	...
	CommandBuffer(VkDevice d, VkQueue q, VkCommandPool commandPool, VkPipelineBindPoint bp = VK_PIPELINE_BIND_POINT_GRAPHICS) {
		device = d;
		queue = q;
...
struct Pipeline {
		...
		uint32_t config_mask = 0x0)
	{
		...
		//Config
		if(config_mask & CONIFG_LINE_STRIP) {
			inputState.topology = VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
		}
