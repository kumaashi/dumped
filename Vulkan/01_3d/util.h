...
struct RenderPass {
	...
	RenderPass(VkDevice device, uint32_t color_num) {
		std::vector<VkAttachmentDescription> attachmentDescs;
		std::vector<VkAttachmentReference>   attachmentRef;
		uint32_t index = 0;
		for(int i = 0 ; i < color_num; i++) {
			attachmentDescs.push_back(AttachmentDescription(VK_FORMAT_B8G8R8A8_UNORM, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_PRESENT_SRC_KHR));
		...
		for(int i = 0; i < attachmentRef.size(); i++) {
			VkClearValue temp;
			temp.color = { { 0.0f, 0.0f, 0.2f, 1.0f } };
